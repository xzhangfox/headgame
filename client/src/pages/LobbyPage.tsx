    ...
    <div className="min-h-screen p-4 flex flex-col items-center justify-center">
      <div className="w-full max-w-2xl">
        ...
        <motion.div className="text-center mb-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-white/40 text-sm">等待玩家加入...</p>
        </motion.div>
        ...
        >
          <p className="text-white/40 text-xs mb-1">房间码</p>
          <div className="flex items-center justify-center gap-3">
            <span className="text-4xl font-black font-mono text-transparent bg-clip-text bg-gradient-to-r from-gold-200 to-gold-500 tracking-widest">
              {room.code}
            </span>
          ...
          <p className="text-white/25 text-xs mt-1">把房间码分享给朋友来加入！</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <AnimatePresence>
                {room.players.map((player, i) => (
                  <motion.div
                    key={player.id}
                    className="flex items-center gap-3 bg-white/5 rounded-xl px-3 py-2.5 border border-gold-700/20"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: 20 }}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white truncate">
                        {player.name}
                        {player.id === myId && (
                          <span className="ml-1.5 text-xs text-gold-400">(你)</span>
                        )}
                      </p>
                    </div>
              ...
              {room.players.length < 2 && (
                <p className="text-center text-white/25 text-xs py-2">至少需要2名玩家</p>
              )}
            </div>
          </motion.div>
          ...
          >
            <h3 className="text-sm font-semibold text-white/50 mb-3">⚙️ 游戏设置</h3>

            {/* Category */}
            <div className="mb-4">
              <label className="text-xs text-white/40 mb-2 block">题库分类</label>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-150 ${
                      settings.category === cat.id
                        ? 'bg-gold-600/30 text-gold-300 ring-1 ring-gold-500/60'
            ...
            {/* Mode */}
            <div className="mb-4">
              <label className="text-xs text-white/40 mb-1.5 block">显示模式</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-white/40 mb-1.5 block">❤️ 生命数</label>
                <select
              ...
              <div>
                <label className="text-xs text-white/40 mb-1.5 block">⏱️ 回合限时</label>
                <select
            ...
            {!isHost && (
              <p className="text-xs text-white/25 mt-3 text-center">等待房主配置设置...</p>
            )}
          </motion.div>
        </div>
        ...
        >
          <button onClick={onLeaveRoom} className="btn-secondary flex-shrink-0">← 离开</button>
          {isHost ? (
            <button
            ...
            <div className="flex-1 glass rounded-xl flex items-center justify-center text-white/35 text-sm">
